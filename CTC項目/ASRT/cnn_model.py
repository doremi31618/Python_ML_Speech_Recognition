...
from readdata25 import DataSpeech
...
class ModelSpeech(): # 语音模型类
    def __init__(self, args):
        ...
        '''
        self.data_args = args
        MS_OUTPUT_SIZE = 1426 #1423pny + PAD + UNK +'_'
    ...
    def CreateModel(self):
        ...
        #loss_out = Lambda(self.ctc_lambda_func,output_shape=(self.MS_OUTPUT_SIZE, ), name='ctc')([y_pred, labels, input_length, label_length])#(layer_h6) # CTC
    ...
    def TrainModel(self, datapath, epoch ,  batch_size , save_step = 1000,filename = abspath + 'model_speech/m' + ModelName + '/speech_model'+ModelName):
        '''
        训练模型
        参数：
            datapath: 数据保存的路径
        ...
        train_mode=DataSpeech(self.data_args , 'train')
        train_mode.LoadDataList()
        dev_mode = DataSpeech(self.data_args, 'dev')
        dev_mode.LoadDataList()
        # data.LoadDataList('dev')
        train_num_data = train_mode.GetDataNum() # 获取数据的数量
        train_step = train_num_data//batch_size
        yielddatas = train_mode.data_genetator(batch_size, self.AUDIO_LENGTH)
        #yielddatas = train_mode.get_am_batch()
        for epoch in range(epoch): # 迭代轮数
            print('[running] train epoch %d .' % epoch)
            n_step = 0 # 迭代数据数
            try:
                print('[message] epoch %d . Have train datas %d+'%(epoch, n_step*train_num_data))
                # data_genetator是一个生成器函数

                #self._model.fit_generator(yielddatas, save_step, nb_worker=2)
                self._model.fit_generator(yielddatas, train_step)
                n_step += 1
            except StopIteration:
                print('[error] generator error. please check data format.')
                break

            self.SaveModel(comment='_e_'+str(epoch)+'_step_'+str(n_step * train_step))
            self.TestModel(self.datapath, mode = train_mode , data_count = 16)
            self.TestModel(self.datapath, mode = dev_mode , data_count = 16)
    ...
    def TestModel(self, datapath='', mode = DataSpeech, data_count = 32, out_report = True, show_ratio = True, io_step_print = 10, io_step_file = 10):
        '''
        测试检验模型效果
        ...
        #data=DataSpeech(self.data_args,self.datapath, mode.GetType())
        # data.LoadDataList(str_dataset)
        num_data = mode.GetDataNum() # 获取数据的数量
        ...
        try:
            ...
            if(out_report == True):
                txt_obj = open('Test_Report_' + mode.GetType() + '_' + nowtime + '.txt', 'w', encoding='UTF-8') # 打开文件并读入

            txt = '测试报告\n模型编号 ' + ModelName + '\n\n'
            for i in range(data_count):
                data_input, data_labels = mode.GetData((ran_num + i + num_bias) % num_data)  # 从随机数开始连续向后取一定数量数据
            ...
            print('*[Test Result] Speech Recognition ' + mode.GetType() + ' set word error ratio: ', word_error_num / words_num * 100, '%')
            if(out_report == True):
                txt += '*[测试结果] 语音识别 ' + mode.GetType() + ' 集语音单字错误率： ' + str(word_error_num / words_num * 100) + ' %'
                txt_obj.write(txt)
                txt = ''
                txt_obj.close()
    ...
    def Predict(self, data_input, input_len):
        ...
        for i in range(batch_size):
            x_in[i,0:len(data_input),:,0] = data_input
