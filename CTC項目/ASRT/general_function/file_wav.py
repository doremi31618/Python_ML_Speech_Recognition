...
def get_wav_list(filename,files):
    '''
    读取一个wav文件列表，返回一个存储该列表的字典类型值
    ps:在数据中专门有几个文件用于存放用于训练、验证和测试的wav文件列表
    '''
    dic_filelist={} # 初始化字典 <class 'dict'>: {'A11_0': 'data_thchs30/train/A11_0.wav'}
    list_wavmark=[] # 初始化wav列表 <class 'list'>: ['A11_0']
    for file in files:
        print('load ', file, ' data...')
        txt_obj=open(filename+file,'r') # 打开文件并读入
        txt_text=txt_obj.read()
        txt_lines=txt_text.split('\n') # 文本分割
        for i in txt_lines: #'A11_0 data_thchs30/train/A11_0.wav'
            if(i!=''):
                txt_l=i.split(' ')
...
def get_wav_han(filename,labels):
    '''
    读取指定数据集中，所有wav文件对应的语音符号
    返回一个存储符号集的字典类型值
    '''
    dic_symbol_list={} # 初始化字典  #<class 'dict'>: {'A11_0': ['lv4', 'shi4', 'yang2', 'chun1', 'yan1', 'jing3', 'da4', 'kuai4', 'wen2', 'zhang1', 'de5', 'di3', 'se4', 'si4', 'yue4', 'de5', 'lin2', 'luan2', 'geng4', 'shi4', 'lv4', 'de5', 'xian1', 'huo2', 'xiu4', 'mei4', 'shi1', 'yi4', 'ang4', 'ran2', '']}
    list_symbolmark=[] # 初始化symbol列表  #<class 'list'>: ['A11_0']
    for file in labels:
        print('load', file, 'hanlist...')

        txt_obj=open(filename+file,'r',encoding='utf-8') # 打开文件并读入
        txt_text=txt_obj.read()
        txt_lines=txt_text.split('\n') # 文本分割  #'A11_0 lv4 shi4 yang2 chun1 yan1 jing3 da4 kuai4 wen2 zhang1 de5 di3 se4 si4 yue4 de5 lin2 luan2 geng4 shi4 lv4 de5 xian1 huo2 xiu4 mei4 shi1 yi4 ang4 ran2 '
        for i in txt_lines:
            if(i!=''):
                txt_l=i.split(' ')
...
def get_wav_symbol(filename,labels):
    '''
    读取指定数据集中，所有wav文件对应的语音符号
    返回一个存储符号集的字典类型值
    '''
